...
public open class UseStreamForAppendTest<R : Storage, S : EventStore, F : StreamTestFactory<R, S>>(
    ...
    public fun `given empty stream - appendStream writes events`() {
        ...
        val appended = eventStore
            ...
            .useStreamForAppend(car1StreamId) { _, appendStream ->
    ...
    @Test
    @JsName("test2")
    public fun `given existing events - block reads stream then appends`() {
        // given
        val storage = factory.newStorage()
    ...
    public fun `given empty stream - block returns without calling appendStream - nothing is persisted`() {
        ...
        eventStore
            .withStreamType(CarStreamType)
            .useStreamForAppend(car1StreamId) { stream, _ ->
    ...
    @Test
    @JsName("test6")
    public fun `sinceVersion drops earlier events from the stream`() {
        // given
        val storage = factory.newStorage()
        val produced = CarProducedEvent(vin = "123", producer = 1, make = "kia", model = "rio")
        val sold = CarSoldEvent(seller = 1, buyer = 2, price = 100.0f)
        ...
        // when
        val streamedSize = eventStore
            .withStreamType(CarStreamType)
            .useStreamForAppend(car1StreamId, sinceVersion = 1) { stream, _ -> stream.toList().size }

        // then
        assertEquals(1, streamedSize)
    }
    ...
    public fun `cached snapshot - sinceVersion skips cached event - then block composes new cache value`() {
        ...
        val newCacheValue = eventStore
            .withStreamType(CarStreamType)
            .useStreamForAppend(car1StreamId, sinceVersion = cache.last().version) { stream, appendStream ->
                val loaded = stream.toList()
                assertEquals(1, loaded.size)
                assertEquals(event2, loaded.single().event)
                cache + loaded + appendStream(listOf<CarEvent>(event3), emptyMap())
            }
        cache = newCacheValue // the cache is only updated after useStreamForAppend returns,
    ...
    @Test
    @JsName("test10")
    public fun `appendStream reflects events consumed lazily from the stream before it was called`() {
        // given
        val storage = factory.newStorage()
        val produced = CarProducedEvent(vin = "123", producer = 1, make = "kia", model = "rio")
        val sold = CarSoldEvent(seller = 1, buyer = 2, price = 100.0f)
        ...
        // when - block only partially consumes the lazy stream before appending
        val appended = eventStore
            .withStreamType(CarStreamType)
            .useStreamForAppend(car1StreamId) { stream, appendStream ->
                val first = stream.first()
                assertEquals(produced, first.event)
                appendStream(listOf<CarEvent>(event3), emptyMap())
            }

        // then - the append version accounts for the whole stream, not just what was consumed
        assertEquals(3, appended.single().version)
    }
}
